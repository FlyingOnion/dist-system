...
func TestReElection2A(t *testing.T) {
	...
	fmt.Println(time.Now(), leader1, "disconnected")
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	cfg.connect(leader1)
	fmt.Println(time.Now(), leader1, "reconnected")
	...
	fmt.Println(time.Now(), leader2, (leader2+1)%servers, "disconnected")
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	cfg.connect((leader2 + 1) % servers)
	fmt.Println(time.Now(), (leader2+1)%servers, "reconnected")
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	cfg.connect(leader2)
	fmt.Println(time.Now(), leader2, "reconnected")
	cfg.checkOneLeader()
